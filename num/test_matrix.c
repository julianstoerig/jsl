...
int main(void) {
    ...
    Mat A = mat_empty(1, 1, ap);
    A.v[0] = 1;
    Mat B = mat_empty(1, 1, ap);
    B.v[0] = 1;
    Mat C = mat_add(A, B, ap);
    if (!isclose(*C.v, 2, 1e-4, 1e-4)) return 1;

    Mat E = vec_row(ap, 0);
    Mat F = mat_mul(A, E, ap);
    if (!mat_equal(E, F, eps, eps)) return 1;

    mat_scale_ip(E, 42);
    if (!mat_equal(E, F, eps, eps)) return 1;

    Mat Id = mat_empty(2, 2, ap);
    at2(Id, 0, 0) = 1; at2(Id, 0, 1) = 0;
    at2(Id, 1, 0) = 0; at2(Id, 1, 1) = 1;
    Mat D = mat_ident(2, 2, ap);
    if (!mat_equal(Id, D, eps, eps)) return 1;

    Mat x = vec_col(ap, 1, 2, 3);
    Mat y = vec_col(ap, 1, 0, 2);
    ...
    if (!mat_equal(mat_augment(K, L, ap), N, eps, eps)) return 1;
    ...
    S64 length = 100;
    Mat z = mat_uniform(length, length, 42, ap);
    Mat w = mat_matmul(z, z, ap);
    mat_print(w);
